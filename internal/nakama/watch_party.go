...
// currentPlaybackMedia returns the media currently being played by the local player, if any.
func (m *Manager) currentPlaybackMedia() (*anilist.BaseAnime, bool) {
	if m == nil || m.playbackManager == nil {
		return nil, false
	}
	return m.playbackManager.GetCurrentMedia()
}

// getLocalMediaIdOfCustomSource re-encodes a remote peer's custom-source MediaId using this
// instance's own local extension identifier for the same extension (identified via the stable
// SiteURL string), since each installation assigns its own local integer id to a given extension.
func (wpm *WatchPartyManager) getLocalMediaIdOfCustomSource(mediaId int, media *anilist.BaseAnime) int {
	...
	extId, ok := customsource.GetCustomSourceExtensionIdFromSiteUrl(media.SiteURL)
	if !ok {
		return mediaId
	}
