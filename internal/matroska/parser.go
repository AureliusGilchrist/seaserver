...
func (mp *MatroskaParser) parseSimpleBlock(size uint64) (*Packet, error) {
	...
	// Read only the header first (track VINT max 8 bytes + 2 timestamp + 1 flags = 11)
	headerSize := int(size)
	if headerSize > 12 {
		headerSize = 12
	}
	...
	if isIgnored {
		// Skip the rest of this SimpleBlock's payload without reading it
	...
	// Reconstruct full data slice for processing
	data := make([]byte, size)
