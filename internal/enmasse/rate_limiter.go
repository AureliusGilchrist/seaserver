...
//
// It also carries the AniList client's own marker, so one call at a handler covers both: the
// larger allowance here, and priority over background work at the client's gate. Two markers
// meaning the same thing that had to be set separately would eventually be set separately.
func WithUserInitiated(ctx context.Context) context.Context {
	return anilist.WithUserInitiated(context.WithValue(ctx, userInitiatedKey{}, true))
}
...
func IsUserInitiated(ctx context.Context) bool {
	...
	return anilist.IsUserInitiated(ctx)
}
