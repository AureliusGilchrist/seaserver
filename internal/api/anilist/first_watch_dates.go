...
//
// So the rule is narrow and the same everywhere: fill a date in when it is empty, never change one
// that is already there, and write nothing at all for a rewatch. The first watch is the one that
// gets recorded, and after that these are somebody's data rather than this server's.
//
...
func FirstWatchDates(entry *AnimeListEntry, isCompleted bool, now time.Time) (startedAt *FuzzyDateInput, completedAt *FuzzyDateInput) {
	// A rewatch records nothing. The dates belong to the first time through, and this is not it.
	if entry != nil && entry.Repeat != nil && *entry.Repeat > 0 {
		return nil, nil
	}
