...
import { useAnimeEntryManualMatch, useAnimeEntryRematch, useFetchAnimeEntrySuggestions } from "@/api/hooks/anime_entries.hooks"
...
export function UnmatchedFileManager(props: UnmatchedFileManagerProps) {
    ...
    const [groups, setGroups] = React.useState(unmatchedGroups)
    ...
    function pruneAndAdvance(removedPaths: string[]) {
        setGroups(prev => {
            const draft = [...prev]
            const current = draft[page]
            if (!current) return prev
            const remainingFiles = current.localFiles?.filter(lf => !removedPaths.includes(lf.path)) ?? []

            if (remainingFiles.length === 0) {
                draft.splice(page, 1)
            } else {
                draft[page] = { ...current, localFiles: remainingFiles }
            }

            const nextPage = Math.min(page, Math.max(0, draft.length - 1))
            setPage(nextPage)
            setCurrentGroup(draft[nextPage])
            setSelectedPaths([])
    ...
    function handleMatchSelected() {
        if (!!currentGroup && anilistId > 0 && selectedPaths.length > 0) {
            const hasMatched = currentGroup.localFiles?.some(lf => lf.mediaId)
            const mutation = hasMatched ? rematch : manualMatch
            mutation({
                paths: selectedPaths,
                mediaId: anilistId,
            ...
            <AppLayoutStack className="mt-4">
                ...
                <div className="bg-gray-950 border p-2 px-2 divide-y divide-[--border] rounded-[--radius-md] max-h-[50vh] max-w-full overflow-x-auto overflow-y-auto text-sm">
                    ...
                    {currentGroup.localFiles?.slice().sort((a, b) =>
                        (Number(a.parsedInfo?.episode ?? 0)) - (Number(b.parsedInfo?.episode ?? 0))
                    )
                        .map((lf, index) => (
                            <div
                                key={`${lf.path}-${index}`}
